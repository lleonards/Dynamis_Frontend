...
import { Lightbulb, ArrowLeft, Clock, TrendingUp, DollarSign, Zap } from 'lucide-react';
...
export default function Luminarias() {
  ...
  const handleCarregar = async () => {
    ...
    try {
      ...
      toast.error('Erro ao carregar luminarias.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '32px 24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '28px' }}>
        ...
        <div>
          <h1 style={{ fontSize: '1.4rem', fontWeight: '700' }}>Tipos de Luminarias</h1>
          <p style={{ color: '#606070', fontSize: '0.85rem' }}>Guia completo: caracteristicas, usos e dicas</p>
        </div>
      </div>
      ...
      {!carregado ? (
        <div className="card" style={{ textAlign: 'center', padding: '40px' }}>
          ...
          <p style={{ color: '#606070', marginBottom: '20px' }}>Guia completo com 10 tipos de luminarias e dicas de instalacao</p>
          <button onClick={handleCarregar} className="btn-primary" disabled={loading} style={{ margin: '0 auto' }}>
            ...
            {loading ? 'Carregando...' : 'Ver guia de luminarias'}
          </button>
          <p style={{ color: '#3a3a50', fontSize: '0.78rem', marginTop: '12px' }}>Consume 1 credito</p>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '12px' }}>
          {luminarias.map((lum, i) => (
            <div
              ...
              style={{
                background: '#141418', border: `1px solid ${expandido === i ? '#b8a07e' : '#2a2a38'}`,
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                ...
                <span style={{
                  ...
                  background: `${custoColor[lum.custo] || '#606070'}15`,
                  color: custoColor[lum.custo] || '#606070',
                  border: `1px solid ${custoColor[lum.custo] || '#606070'}30`
                }}>{lum.custo}</span>
              </div>
              <div style={{ display: 'flex', gap: '8px', marginBottom: '8px', flexWrap: 'wrap' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.78rem', color: '#a0a0b0' }}>
                  <Zap size={11} style={{ color: '#e0a852' }} />{lum.potencia}
                </span>
                <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.78rem', color: '#a0a0b0' }}>
                  <Clock size={11} style={{ color: '#4a7fa5' }} />{lum.vida_util}
                </span>
              </div>
              ...
              {expandido === i && (
                <div style={{ borderTop: '1px solid #2a2a38', paddingTop: '12px', marginTop: '4px' }}>
                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center', marginBottom: '8px' }}>
                    ...
                    <span style={{ fontSize: '0.8rem', color: '#a0a0b0' }}>Eficiencia: <strong style={{ color: '#4caf82' }}>{lum.eficiencia}</strong></span>
                  </div>
