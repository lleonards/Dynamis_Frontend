...
function buildLabel(ambientes, entries = []) {
  const numeroEstudo = entries.length + 1;

  if (ambientes.length === 1) {
    const numeroAmbiente = getSavedAmbientesCount(entries) + 1;
    return `Estudo ${numeroEstudo} • Ambiente ${numeroAmbiente}`;
  }

  return `Estudo ${numeroEstudo} • ${ambientes.length} ambientes`;
}
...
export default function CalcMateriais() {
  ...
  const handleSave = () => {
    ...
    const label = editingId ? (existingEntry?.label || buildLabel(ambientes, entries)) : buildLabel(ambientes, entries);

    saveToolEntry('materiais', { ambientes }, label, editingId);
    refreshEntries();
    resetCurrent();
