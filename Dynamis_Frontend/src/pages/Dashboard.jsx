            ...
            {!creditHistory.length ? (
              ...
              <div style={{ display: 'grid', gap: '10px' }}>
                {creditHistory.slice(0, 8).map((item) => (
                  <div key={item.id} style={{ background: '#141418', border: '1px solid #2a2a38', borderRadius: '10px', padding: '12px' }}>
                    <strong style={{ color: '#fff', fontSize: '0.9rem' }}>{item.ferramenta}</strong>
                    <div style={{ color: '#606070', fontSize: '0.8rem', marginTop: '6px' }}>{new Date(item.created_at).toLocaleString('pt-BR')}</div>
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '8px' }}>
                      <span className="badge badge-neutral">Antes: {item.creditos_antes}</span>
                      <span className="badge badge-green">Depois: {item.creditos_depois}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {currentProject && (
          <div>
            <SectionHeader
              ...
              title={`Projeto atual: ${currentProject.nome_projeto}`}
            ...
            <div className="card" style={{ marginBottom: '18px' }}>
              <div className="result-grid">
                ...
                <div className="result-item"><div className="value">{currentProject.total_ferramentas}</div><div className="label">Ferramentas processadas</div></div>
                <div className="result-item"><div className="value">{(currentProject.ferramentas_geradas || []).reduce((acc, item) => acc + item.quantidade, 0)}</div><div className="label">Resultados exibidos</div></div>
              </div>
            </div>

            {Object.entries(currentProject.resultados || {}).map(([toolKey, records]) => (
              <div key={toolKey} className="card" style={{ marginBottom: '16px' }}>
                <SectionHeader
